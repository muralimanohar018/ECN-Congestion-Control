...
void EcnMonitor::OnMark(Ptr<const QueueDiscItem> item, const char* reason)
{
    ...
    std::cout << "[ECN MARK] "
              << "time=" << std::fixed << std::setprecision(6) << Simulator::Now().GetSeconds()
              << " s"
              << " | total=" << g_totalPackets
              << " | marked=" << g_ecnMarkedPackets
              << " | ratio=" << std::setprecision(8) << GetRatio()
              << " | reason=" << reason
              << "\n";
}
...
double EcnMonitor::GetRatio()
{
    ...
    return static_cast<double>(g_ecnMarkedPackets) / g_totalPackets;
}
